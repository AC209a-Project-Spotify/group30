...
We have successfully built a regression model to predict the number of followers of a playlist with reasonable out-of-sample $R^2$ scores (**Stacked Meta Linear Regression model**  with a test $R^2$ score of 0.49653 with main predictors only and that of 0.49444 with main predictors + interaction terms between genre and audio features). We have also built a recommender system based on user-specified inputs. We found that the performance of the recommender system varies greatly for different user inputs. One possible reason is that most of the playlists collected are of the genre “pop”. To mitigate this problem, we could consider collecting more balanced number of playlists/tracks among all genres or consider a better categorization of playlist genres (Note: due to a lack of data labeling, we used most frequently occurring artist genre as playlist genre). Another possible reason for the variance in recommender system performance is that we filtered track candidates based on popularity when popularity only weakly associates with number of followers. 

We envision 3 directions for **future work**:

- Improve the performance of our predictive model and recommender system — As described above, possible approaches include collecting a training set of playlists with more balanced genre; improving strategy to label playlist and track genres, optimizing track search algorithm; and optimizing similarity metric.


- Develop automatic filtering — To make our recommender system more user-friendly, we envision building another machine learning algorithm to extract user preferences based on usage history. The learned usage patterns, then, can be used to complement explicit user input.
- Integrate our recommender system onto the Spotify platform 